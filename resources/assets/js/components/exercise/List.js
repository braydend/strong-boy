...
export function List()
{
	...
	const updateCards = () => {
		...
		axios.get("/ajax/exercise")
			.then((res) => {
				setExercises(res.data.map((obj, i) => <ExerciseCard exercise={obj} key={i} />));
				setIsLoading(false);
			})
			.catch((error) => {
				setExercises(
	...
	useEffect(() => { if(!init){updateCards(); setInit(true);} });

	return (
		<div className="exerciseList">
			<div className="quickAdder">
				<InputGroup>
					<FormControl id="exercise-search-adder" onChange={() => searchCards()} placeholder="Exercise name" />
					<InputGroup.Append>
						<Button onClick={() => exerciseSearchAdder()} variant="outline-secondary" disabled={isLoading}>
							<i id="exercise-search-adder-btn" className="fas fa-search" hidden={isLoading} />
							<Spinner animation="border" variant="secondary" as="span" size="sm" hidden={!isLoading} />
